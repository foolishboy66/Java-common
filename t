...
public class HttpUtils {
	...
	static {
		...
		ConnectionSocketFactory ssLConnectionSocketFactory = new SSLConnectionSocketFactory(sslContext,
		...
		RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(CONNECTION_TIMEOUT_IN_MILLS)
				.setSocketTimeout(SOCKET_TIMEOUT_IN_MILLS)
				.setConnectionRequestTimeout(CONNECTION_REQUEST_TIMEOUT_IN_MILLS).build();
	...
	public static <T> T get(String url, String describe, TypeReference<T> type) {

		return get(url, null, null, describe, true, true, type);
	}

	public static <T> T get(String url, String describe, boolean logRequest, boolean logResponse,
			TypeReference<T> type) {

		return get(url, null, null, describe, logRequest, logResponse, type);
	}

	public static <T> T get(String url, Map<String, String> header, String describe, boolean logRequest,
			boolean logResponse, TypeReference<T> type) {

		return get(url, null, header, describe, logRequest, logResponse, type);
	}

	public static <T> T get(String url, Map<String, String> args, Map<String, String> header, String describe,
			boolean logRequest, boolean logResponse, TypeReference<T> type) {

		String response = get0(url, args, header, describe, logRequest, logResponse);
		if (StringUtils.isBlank(response)) {
			return null;
		}
		return JsonUtils.toObj(response, type);
	}

	public static <T> T post(String url, Object entity, String describe, TypeReference<T> type) {

		return post(url, entity, null, describe, true, true, type);
	}

	public static <T> T post(String url, Object entity, String describe, boolean logRequest, boolean logResponse,
			TypeReference<T> type) {

		return post(url, entity, null, describe, logRequest, logResponse, type);
	}
	...
	private static String execute(HttpUriRequest request) throws ClientProtocolException, IOException {
